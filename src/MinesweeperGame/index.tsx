...
export default class Minesweeper extends React.Component<IProps, IState> {
    ...
    componentDidMount() {
        ...
        let boardValues: Array<Array<GameObject>> = Array.apply(null, Array(rows)).map(() => Array.apply(null, Array(columns)).map(() => {})),
            boardState: Array<Array<CellState>> = Array.apply(null, Array(rows)).map(() => Array.apply(null, Array(columns)).map(() => "C"))
        ...
        boardValues = boardValues.map((row, rowIndex) => 
            row.map((cell, columnIndex) => {
                if(cell != "M") {
                    return this.getNumberForCell(boardValues, rowIndex, columnIndex)
                } else {
                    return cell
                }
            })
    ...
    getNumberForCell = (boardValues: Array<Array<GameObject>>, row: number, column: number): number => {
        let count = 0;
    ...
    markCell = (boardState: Array<Array<CellState>>, rowIndex: number, columnIndex: number) => {
        switch(boardState[rowIndex][columnIndex]) {
            case "C":
                boardState[rowIndex][columnIndex] = "F";
                this.setState(
                    ({ cellsRemeining, minesRemaining }) => ({
    ...
    revealCell = (boardState: Array<Array<CellState>>, rowIndex: number, columnIndex: number) => {
        ...
        if(boardValues[rowIndex][columnIndex] == "M") {
            ...
            if(columnIndex < columns - 1 && boardState[rowIndex][columnIndex + 1] == "C")
                this.revealCell(boardState, rowIndex, columnIndex + 1);

            if(rowIndex < rows - 1 && boardState[rowIndex + 1][columnIndex] == "C")
                this.revealCell(boardState, rowIndex + 1, columnIndex);

            if(rowIndex > 0 && boardState[rowIndex - 1][columnIndex] == "C")
                this.revealCell(boardState, rowIndex - 1, columnIndex);

            if(columnIndex > 0 && rowIndex > 0 && boardState[rowIndex - 1][columnIndex - 1] == "C")
                this.revealCell(boardState, rowIndex - 1, columnIndex - 1);

            if(columnIndex > 0 && rowIndex < rows - 1 && boardState[rowIndex + 1][columnIndex - 1] == "C")
                this.revealCell(boardState, rowIndex + 1, columnIndex - 1);

            if(columnIndex < columns - 1 && rowIndex < rows - 1 && boardState[rowIndex + 1][columnIndex + 1] == "C")
                this.revealCell(boardState, rowIndex + 1, columnIndex + 1);

            if(columnIndex < columns - 1 && rowIndex > 0 && boardState[rowIndex - 1][columnIndex + 1] == "C")
                this.revealCell(boardState, rowIndex - 1, columnIndex + 1);
        }
    }
    ...
    componentDidUpdate() {
        if(this.state.cellsRemeining == this.state.minesRemaining && this.state.gameResult !== "WIN") {
                            ...
                            row.map((cell, cellIndex) => {
                                ...
                                return(
                                    <div
                                        ...
                                        onMouseUp={ e => this.onCellСlick(e, rowIndex, cellIndex) }
